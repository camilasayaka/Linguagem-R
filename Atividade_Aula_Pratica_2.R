##### ME115 - Linguagem R #####
##### Atividade - Aula pratica 2 #####
##### Nome: Camila Sayaka Hiura, RA: 167423 #####
...
soma_linha2 <- X[2,1]+X[2,2]+X[2,3]
soma_linha2

#(b) a soma dos elementos da coluna 3.
soma_coluna3 <- X[1,3]+X[2,3]+X[3,3]
soma_coluna3

#(c) a soma dos elementos da diagonal de X.
soma_diagonal <- X[1,1]+X[2,2]+X[3,3]
soma_diagonal
...
set.seed(12345)
y <- sample(seq(2,1000,2), size=10, replace=T)
y
if (all(y%%2==0)) {
  print("Todos são números pares.")
} else {
